...
def main():
    ...
    if not distributed:
        ...
        # === 🎯 Background (class 0) → 255 (ignore index) for mIoU ===
        # if args.eval and 'mIoU' in args.eval:
        #     new_results = []
        #     for p in results:
        #         if isinstance(p, tuple) and isinstance(p[0], np.ndarray):
        #             # 유지: (prediction, meta) 구조
        #             pred = np.where(p[0] == 0, 255, p[0]).astype(np.uint8)
        #             new_results.append((pred, *p[1:]))
        #         elif isinstance(p, np.ndarray):
        #             # 단일 prediction만 있을 경우
        #             pred = np.where(p == 0, 255, p).astype(np.uint8)
        #             new_results.append(pred)
        #         else:
        #             new_results.append(p)
        #     results = new_results
        # if isinstance(results[0], tuple):
        #     print("Unique pred values after post-processing:", np.unique(results[0][0]))
        # else:
        #     print("Unique pred values after post-processing:", np.unique(results[0]))
    else:
